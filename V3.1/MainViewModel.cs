    ...
    class MainViewModel : ViewModel
    {
        ...
        private void _serv_MessageReceived(string msg)
        {
            ...
            try
            {
                ...
                try
                {
                    _dba.InsertData(TempAktuellValue, FeuchteAktuellValue, PressCurrentVal, DateTime.Now.ToString("yyyy-MM-dd hh:mm:ss")/*DateTime.Now.ToString(string.Format("yy-MM-dd hh:mm:ss.ffff"),cultures)*/, msg.Split(';')[3]);
                }
                catch (Exception ex)
                {
                    Status = ex.Message;
